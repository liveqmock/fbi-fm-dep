...
@ManagedBean
@ViewScoped
public class BiDataFlowAction {

    private static Logger logger = LoggerFactory.getLogger(BiDataFlowAction.class);
    ...
    public void init() {
        ...
        //bankCodeList = toolsService.getEnuSelectItemList("BANKCODE", true, false);
    ...
    private void createPieModel() {
        if(notifyMsgCntList != null && !notifyMsgCntList.isEmpty()) {
            for(BiNotifyMsgCnt bnmc : notifyMsgCntList) {
                pieModel.set(bankMap.get(bnmc.getBankcode()), bnmc.getCount());
            }
        }
